...
int pivot(publicidad **p1, int izq, int der)
{
    ...
    pivote = izq;
    valor_pivote = p1[0][pivote].valor;
    for (i=izq+1; i<=der; i++){
        if (p1[0][pivote].valor > valor_pivote){
                pivote++;
                aux=*p1[i];
                *p1[i]=*p1[pivote];
                *p1[pivote]=aux;
...
int MAX(lista numeros)
{
	int i=0;
	int max = numeros.arreglo[0].elemento ;
	for(i=0;i<numeros.fin;i++)
	{
		if(numeros.arreglo[i].elemento>max)
		{
			max = numeros.arreglo[i].elemento;
		}
	}
	return max;
}
...
resultadoPag * insertarPublicidad(publicidad * publicidades, int numPub, pagina * paginas, int numPag )
{
	...
	while(pag < numPag )
	{
		...
		{	
			...
			{
				largos = append(largos,publicidades[pub].largo);
				anuncios+=1;
				...
				anchoActual-=publicidades[pub].ancho;
				buscarAncho=1;
				while(buscarAncho)
				{
					if(usados.arreglo[pub].elemento == 0 && publicidades[pub].largo <= largoActual && publicidades[pub].ancho <= anchoActual)
					{
						printf("ingresandoPUB alo ancho %i\n",pub);
					...
					if(pub>numPub)
					{
						largoMaximo = MAX(largos);
						largoActual -= largoMaximo;
						buscarAncho=0;
						pub=0;
					}		
				}
			...
			{
				printf("largoPub %d largoActual %d anchoPub %d anchoPagina %d\n",publicidades[pub].largo, largoActual, publicidades[pub].ancho, paginas[pag].ancho);
				pub +=1;
